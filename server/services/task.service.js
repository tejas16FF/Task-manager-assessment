...
async function getTask(taskId, currentUser) {
  const task = await Task.findById(taskId).populate(populateTaskUsers);
...
async function createTask(payload, currentUser) {
  ...
  const project = await findOrCreateProjectByName(
  ...
  io.to(`user-${normalizedPayload.assignedTo}`).emit("notification", {
    type: "TASK_ASSIGNED",
    title: "New task assigned",
...
async function applyTaskStatus(task, nextStatus, currentUser) {
  ...
  let activityAction = ACTIVITY_ACTIONS.TASK_UPDATED;
  let activityMessage = `Task "${task.title}" updated`;
  ...
  if (nextStatus === TASK_STATUS.PENDING) {
    ...
    activityMessage = `Task "${task.title}" reopened`;
  }
  ...
  if (nextStatus === TASK_STATUS.IN_PROGRESS) {
    ...
    task.completedAt = undefined;
    activityAction = ACTIVITY_ACTIONS.TASK_STARTED;
    activityMessage = `Task "${task.title}" started`;
  }
  ...
  if (nextStatus === TASK_STATUS.COMPLETED) {
    ...
    if (task.startedAt) {
      task.totalTimeTaken = Math.floor(
        (task.completedAt - task.startedAt) / 1000
      );
    }

    activityAction = ACTIVITY_ACTIONS.TASK_COMPLETED;
    activityMessage = `Task "${task.title}" completed`;
  }
  ...
  if (task.projectRef) {
    sideEffects.push(recalculateProjectStats(task.projectRef, task.project));
  }
...
async function moveTaskStatus(taskId, nextStatus, currentUser) {
  ...
  return applyTaskStatus(task, nextStatus, currentUser);
}

async function toggleComplete(taskId, currentUser) {
  const task = await Task.findById(taskId);
